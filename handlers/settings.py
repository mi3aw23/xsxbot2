...
# ─── Set Dev Info — only DEV_ID ───────────────────────────────────────────────

async def start_set_dev(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_lang_for_user(user_id)
    if user_id != DEV_ID:
        await update.message.reply_text(get_text("not_authorized", lang))
        return
    db.set_state(user_id, "waiting_dev_name")
    await update.message.reply_text(
        get_text("send_dev_name", lang),
        reply_markup=cancel_keyboard(lang),
        parse_mode=ParseMode.HTML
    )
...
async def receive_dev_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state != "waiting_dev_name":
        return
    data["dev_name"] = update.message.text.strip()
    db.set_state(user_id, "waiting_dev_id", data)
    await update.message.reply_text(get_text("send_dev_id", lang))


async def receive_dev_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_lang_for_user(user_id)
    state, data = db.get_state(user_id)
    if state != "waiting_dev_id":
        return
    try:
        data["dev_id"] = int(update.message.text.strip())
    except ValueError:
        await update.message.reply_text(get_text("invalid_time", lang))
        return
    db.set_state(user_id, "waiting_dev_username", data)
    await update.message.reply_text(get_text("send_dev_username", lang))


async def receive_dev_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_lang_for_user(user_id)
    state, data = db.get_state(user_id)
    if state != "waiting_dev_username":
        return
    data["dev_username"] = update.message.text.strip().lstrip("@")
    db.set_state(user_id, "waiting_dev_photo", data)
    await update.message.reply_text(get_text("send_dev_photo", lang))


async def receive_dev_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    lang = get_lang_for_user(user_id)
    state, data = db.get_state(user_id)
    if state != "waiting_dev_photo":
        return

    msg = update.message
    photo_file_id = None
    if msg.text and msg.text.strip() == "/skip":
        photo_file_id = db.get_setting("dev_photo_file_id")
    elif msg.photo:
        photo_file_id = msg.photo[-1].file_id

    db.set_setting("dev_name", data.get("dev_name", "Dev"))
    db.set_setting("dev_id", str(data.get("dev_id", DEV_ID)))
    db.set_setting("dev_username", data.get("dev_username", ""))
    if photo_file_id:
        db.set_setting("dev_photo_file_id", photo_file_id)

    db.clear_state(user_id)
    await msg.reply_text(
        get_text("dev_info_saved", lang),
        reply_markup=settings_keyboard(lang, user_id=user_id),
        parse_mode=ParseMode.HTML
    )
